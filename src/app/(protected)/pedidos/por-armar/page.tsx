...
export default function PedidosPorArmar() {
  useUserOrRedirect();
  const { orders, loading } = usePedidos();
  const filtered = (orders || []).filter((o: unknown) => (o as any).o_localStatus === "pending_assembly"); // eslint-disable-line @typescript-eslint/no-explicit-any
