...
export default function PedidosPorRecoger() {
  useUserOrRedirect();
  const { orders, loading } = usePedidos();
  const filtered = (orders || []).filter((o: unknown) => (o as any).o_localStatus === "pending_pick_up"); // eslint-disable-line @typescript-eslint/no-explicit-any
