...
export default function UbicacionesPage() {
  ...
  const fetchLocations = async () => {
    try {
      const response = await fetch(getApiUrl('/order-location?limit=100&page=1'), {
  ...
  const onSubmit = async (data: LocationFormData) => {
    ...
    try {
      const url = editingLocation 
        ? getApiUrl(`/order-location/${editingLocation.id}`)
        : getApiUrl('/order-location');
            ...
            <div className="relative group">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Ej: Nevera numero 1 - puerta 1"
                />
              </div>
              {errors.name && (
